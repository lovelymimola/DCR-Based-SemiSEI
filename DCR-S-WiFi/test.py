...
def test(model, test_dataloader, rand_num):
    ...
    writer = pd.ExcelWriter("ML-FixMatch_20label_rand"+str(rand_num)+"/Y_pred.xlsx")
    ...
    writer = pd.ExcelWriter("ML-FixMatch_20label_rand"+str(rand_num)+"/Y_real.xlsx")
...
def main():
    ...
    model = torch.load("model_weight/CNN_FixMatch_SSML_Norm_Rotate2_n_classes_16_20label_80unlabel_rand"+str(rand_num)+".pth")
    print(model)
