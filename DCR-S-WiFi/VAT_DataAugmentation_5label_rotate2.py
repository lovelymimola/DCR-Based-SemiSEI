...
def TrainDataset(ft, random_num):
    x = np.load(f"/data/fuxue/WiFi_ft62/Feet{ft}_X_train.npy")
    y = np.load(f"/data/fuxue/WiFi_ft62/Feet{ft}_Y_train.npy")
    y = y.astype(np.uint8)
    ...
    X_train_labeled4, X_train_unlabeled4, Y_train_labeled4, Y_train_unlabeled4 = train_test_split(X_train_labeled3,Y_train_labeled3,test_size=0.5,random_state=random_num)

    X_train_label, X_val, Y_train_label, Y_val = train_test_split(X_train_labeled4, Y_train_labeled4, test_size=0.3, random_state=random_num)

    X_train_unlabeled = np.concatenate((X_train_unlabeled1,X_train_unlabeled2,X_train_unlabeled3,X_train_unlabeled4), axis=0)
    Y_train_unlabeled = np.concatenate((Y_train_unlabeled1, Y_train_unlabeled2,Y_train_unlabeled3,Y_train_unlabeled4), axis=0)
...
def TestDataset(ft):
    x = np.load(f"/data/fuxue/WiFi_ft62/Feet{ft}_X_test.npy")
    y = np.load(f"/data/fuxue/WiFi_ft62/Feet{ft}_Y_test.npy")
    y = y.astype(np.uint8)
    return x, y
