...
from VAT_DataAugmentation_10label_rotate2 import *
...
def train(model, loss_center, train_dataloader, VAT_dataloader, optimizer, optimizer_cent, threshold, epoch, writer, device_num, batch_size):
    ...
    for num, (data_nnl, data_vat) in enumerate(zip(train_dataloader,VAT_dataloader)):
        ...
        center_loss_u_s_batch = 0.0 * loss_center(feature_x_norm, targets_x_w)[1]
        if epoch >= 100:
            if len(we_can_believe) >= 2:
                feature_norm_u_w = F.normalize(feature_u_w, p=2, dim=1)
                feature_norm_u_s = F.normalize(feature_u_s, p=2, dim=1)
                center_loss_u_w_batch = loss_center(feature_norm_u_w[we_can_believe], targets_u[we_can_believe])[1]
                center_loss_u_s_batch = loss_center(feature_norm_u_s[we_can_believe], targets_u[we_can_believe])[1]

        weight_cent = 0.001
        lambda_u = 1
        result_loss_batch = Lx + lambda_u * Lu + weight_cent * (cent_loss_x_batch + center_loss_u_w_batch + center_loss_u_s_batch)
        ...
        cent_loss_u_s += center_loss_u_s_batch.item()
    ...
    cent_loss_u_s /= len(train_dataloader)
    result_loss /= num

    print('Train Epoch: {} \tLabel_Classifier_Loss: {:.6f}, Unlabel_Classifier_Loss: {:.6f}, Label Center_Loss: {:.6f}, Unlabel Center_Loss (W): {:.6f}, Unlabel Center_Loss (S): {:.6f}, Combined_Loss: {:.6f}, Accuracy: {}/{} ({:0f}%)\n'.format(
        epoch,
...
class Config:
    def __init__(
        ...
        lr_cent: float = 0.001,
        n_classes: int = 16,
        save_path: str = 'model_weight/CNN_FixMatch_SSML_Norm_Rotate2_n_classes_16_10label_90unlabel_rand30.pth',
...
def main():
    conf = Config()
    writer = SummaryWriter("logs_FixMatch_SSML_Norm_Rotate2_10label_rand30")
