...
def train(model, train_dataloader, VAT_dataloader, optimizer, threshold, epoch, writer, device_num, batch_size):
    model.train()
    ...
    for num, (data_nnl, data_vat) in enumerate(zip(train_dataloader,VAT_dataloader)):
        ...
        lambda_u = 1
        result_loss_batch = Lx + lambda_u * Lu
    ...
    print('Train Epoch: {} \tLabel_Classifier_Loss: {:.6f}, Unlabel_Classifier_Loss: {:.6f}, Combined_Loss: {:.6f}, Accuracy: {}/{} ({:0f}%)\n'.format(
        epoch,
...
def train_and_test(model, train_dataset, VAT_dataset, val_dataset, optimizer, threshold, epochs, writer, save_path, device_num, batch_size, vat_batch_size):
    ...
    for epoch in range(1, epochs + 1):
        ...
        train(model, train_dataloader, VAT_dataloader, optimizer, threshold, epoch, writer, device_num, batch_size)
...
class Config:
    def __init__(
        ...
        lr: float = 0.001,
        n_classes: int = 16,
        save_path: str = 'model_weight/CNN_FixMatch_Rotate2_n_classes_16_10label_90unlabel_rand30.pth',
...
def main():
    conf = Config()
    writer = SummaryWriter("logs_FixMatch_Rotate2_10label_rand30")
