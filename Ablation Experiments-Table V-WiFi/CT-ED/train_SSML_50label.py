...
def train(model, loss_center, train_dataloader, VAT_dataloader, optimizer, optimizer_cent, threshold, epoch, writer, device_num, batch_size):
    ...
    for num, (data_nnl, data_vat) in enumerate(zip(train_dataloader,VAT_dataloader)):
        ...
        center_loss_u_w_batch = 0.0 * loss_center(feature_x, targets_x_w)[1]
        if epoch >= 100:
...
def train_and_test(model, loss_center, train_dataloader, VAT_dataloader, val_dataloader, optimizer, optimizer_cent, threshold, epochs, writer, save_path, device_num, batch_size):
...
class Config:
    def __init__(
        ...
        lr_cent: float = 0.001,
        n_classes: int = 16,
        save_path: str = 'model_weight/CNN_FixMatch_SSML_Norm_Rotate2_n_classes_16_50label_50unlabel_rand30.pth',
...
def main():
    conf = Config()
    writer = SummaryWriter("logs_FixMatch_SSML_Norm_Rotate2_50label_rand30")
